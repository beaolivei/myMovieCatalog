...
class MovieListMostPopular extends React.Component {
    ...
    render () {
        console.log('dataMovie', this.state.dataMovie.results)
        const entries = Object.values(this.state.dataMovie)
        const myTitles = entries.map(title => title)
        console.log('entries', entries[3])
        console.log('myTitle', myTitles)
